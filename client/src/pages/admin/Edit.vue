    ...
    <div>
        <div class="absolute w-full left-0 top-0 p-6 flex justify-between items-center space-x-6">
            ...
            <div class="flex items-center space-x-6">
                ...
                <router-link :to="{ name: 'post' }" class="text-sm font-medium text-gray-800">
                    Preview
                </router-link>
            </div>
        </div>
        <ResizeTextArea v-if="post.title">
            <template v-slot:default="{ resize, el}">
                <textarea
                    ...
                    v-model="post.title"
                    v-on:input="resize"
                    class=" w-full text-center text-4xl lg:text-6xl leading-10 font-extrabold tracking-tight border-none focus:ring-0 resize-none p-0"
                    rows="1"
                >
